...
//
// "inlineEditable": campurile FARA nicio verificare de conflict/auto-creare
// (text simplu sau numar) se pot edita direct in celula, ca la Excel - click,
// scrii, Enter/blur salveaza. Data/ora, Trainer, Sala, Responsabil raman
// editabile DOAR prin formularul complet (au verificari - suprapunere,
// auto-creare nume noi - care nu au sens simplificate intr-o singura celula).
export const ATTRIBUTE_COLUMN_OPTIONS = [
  ...
  { key: 'invite_mail', label: 'Mail invitare', getValue: (c) => c.invite_mail || '', inlineEditable: true, field: 'invite_mail', inputType: 'text' },
  { key: 'catering', label: 'Catering', getValue: (c) => c.catering || '', inlineEditable: true, field: 'catering', inputType: 'text' },
  { key: 'notes', label: 'Observatii', getValue: (c) => c.notes || '', inlineEditable: true, field: 'notes', inputType: 'text' },
]
...
export default function WeekGrid({
  ...
  function startEdit(course, col) {
    setEditingCell({ courseId: course.id, key: col.key })
    setDraft(col.getValue(course))
  }
  function cancelEdit() {
    setEditingCell(null)
    setDraft('')
  }
  async function saveEdit(course, col) {
    ...
    setEditingCell(null)
    setDraft('')
    if (!error) onInlineUpdated?.()
    // o eroare aici e foarte improbabila (campuri text simple, fara
    // verificari) - daca totusi apare, valoarea veche ramane afisata la
    // urmatoarea reincarcare, fara sa blocheze nimic
  }
  function handleEditKeyDown(e, course, col) {
    if (e.key === 'Enter') { e.preventDefault(); saveEdit(course, col) }
            ...
            {weekCourses.map((c, rowIndex) => (
              <Fragment key={c.id}>
                ...
                {activeAttrColumns.map((col, colIndex) => {
                  ...
                  const isEditing = editingCell?.courseId === c.id && editingCell?.key === col.key
                  if (isEditing) {
                    return (
                      <div
                        key={`${c.id}-${col.key}`}
                        className="week-attr-cell week-attr-cell-editing"
                      ...
                      >
                        <input
                          ...
                          onKeyDown={(e) => handleEditKeyDown(e, c, col)}
                          onBlur={() => saveEdit(c, col)}
                          onClick={(e) => e.stopPropagation()}
                        />
                      </div>
                    )
                  ...
                  return (
                    <div
                      key={`${c.id}-${col.key}`}
                      className={`week-attr-cell ${c.id === hoveredCourseId ? 'week-attr-cell-highlighted' : ''} ${c.cancelled ? 'week-attr-cell-cancelled' : ''} ${col.inlineEditable ? 'week-attr-cell-inline' : ''}`}
                      style={{ gridColumn: DAY_COLS + colIndex + 1, gridRow: rowIndex + 2 }}
                      title={col.inlineEditable ? 'Click pentru editare rapida' : (value || undefined)}
                      onClick={() => (col.inlineEditable && !c.cancelled ? startEdit(c, col) : onCourseClick(c))}
