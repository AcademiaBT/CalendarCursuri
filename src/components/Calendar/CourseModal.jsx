...
export default function CourseModal({ initialDate, course, onClose, onSaved, initialCloneMode = false }) {
  ...
  const isExistingCourse = Boolean(course?.id)
  // "clona" transforma temporar modalul din "editare" in "adaugare noua",
  // pastrand toate valorile completate ca punct de plecare - restul
  // codului (isEditing) trateaza automat clona ca pe un curs nou, fara sa
  // fie nevoie de nicio alta schimbare (insert in loc de update la Salveaza,
  // butonul Sterge dispare, "Curs anulat" dispare, etc.)
  const [isCloning, setIsCloning] = useState(false)
  const isEditing = isExistingCourse && !isCloning
  ...
  // Clonare: toate campurile raman identice cu cursul curent, in afara de
  // data de start/sfarsit (resetate la azi, cel mai simplu punct de
  // plecare - userul le ajusteaza dupa) si "Curs anulat" (o clona porneste
  // mereu activa, indiferent daca originalul era anulat). Nu se salveaza
  // nimic inca - transforma doar modalul intr-un "Adauga curs" precompletat,
  // userul apasa Salveaza cand e multumit de date.
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="course-form">
          ...
          <div className="modal-actions span-2">
            ...
            {isEditing && (
              <button type="button" className="secondary-btn" onClick={handleClone} disabled={busy}>
                Cloneaza curs
              </button>
            )}
